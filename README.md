...
- ~~replace fixed sample generators with state-driven player policies that react to pool prices, inventory, and spreads instead of emitting hard-coded intents~~
...
- later make player policies become network-aware, for example arbitrage bots reacting to queue depth, skip rates, recent validator misses, congestion, and fork risk instead of only market state.
- later make validator policies state-driven too, for example maintenance/outage behavior, adaptive skip behavior, stake growth or exit decisions, and reactions to congestion or profitability.
...
- ~~add one configurable state-driven policy per current player type while keeping policy choice separate from player type~~
